...
if(!is.null(year_data) && nrow(year_data) >0) {
  year_data <- filter(year_data, events == "home_run")
  yearly_data[[i]] <- year_data
year_data <- as.data.frame(fetch_hr_year(year))
year_data <- clean_hr_data(year_data, am)
}
